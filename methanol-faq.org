...
First Posted: 2025.07.07, [[https://github.com/nworbmot/nworbmot-blog][Last Revised]]: 2026.05.14, Author: [[https://www.nworbmot.org/][Tom Brown]]

This Frequently Asked Questions page answers common questions arising
from our 2026 paper in Joule [[https://doi.org/10.1016/j.joule.2026.102464][A Minimal Methanol Backstop for High
Electrification Scenarios]] (see [[./world-without-gas.html][this blog post]] and [[./minimal-methanol-economy.html][this one]] for an
...
The basic concept is described in these blog posts:

[[./world-without-gas.html][A world without gas: electrification with a minimal methanol backstop]]
...
in Europe in 2050, there is 980 TWh/a of electrolytic hydrogen
production:
...
biomass), ammonia (using nitrogen from the air in the Haber-Bosch
process) and direct reduced iron for steel (using imported iron ore).
...
At 400 MtCO_2/a sequestration the use of electrolytic hydrogen drops to 389 TWh/a:
...
With infinite sequestration it drops to 239 TWh/a:
...
Using methane instead of methanol would allow more of current fossil
gas supply to be directly replaced, and would reuse more of the
existing methane infrastructure (pipelines, storage, end-use devices).

Our scenario CH_4 shows that using methane as the backup energy
carrier and excluding hydrogen would only be 0.8% more expensive than
the optimum:

[[./graphics/methanol_faq-260330/methanol-cost_with_caption.png]]

This would also avoid the complexities of building up a new hydrogen infrastructure.

However, we would argue that:
- Methane still requires substantial infrastructure (pipelines, underground storage).
- Methanol can be bunkered cheaply in aboveground tanks, which is good for resilience (power plants are not dependent on the functioning of the gas network) and is good for countries with no underground gas storage (e.g. Ireland, Switzerland, and more).
- Future methanol demand is much more robust than methane demand in
  climate neutral scenarios, since methanol is used as an industry
  feedstock, shipping and for kerosene production. All of our scenarios are methanol-heavy scenarios.
- Methanol is a much better shipping fuel than liquefied methane, since it has lower volume, no cooling requirements, and there is no gas slippage from engines (methane is a greenhouse gas).
...
* Wouldn't abandoning methane and fossil fuels leave many stranded infrastructure assets?
  :PROPERTIES:
  :CUSTOM_ID: stranded
  :END:

For all the scenarios there is a risk of stranded assets in some industry sectors as
well as for gas distribution due to declining use of fossil fuels. However, if managed well, all assets
can be retired before they reach their end of life or are fully depreciated. In the steel and basic
chemicals sectors in Europe, 90% of blast furnaces need relining between 2025 and 2040 while 41%
of steam crackers reach their end of life before 2030 ([[https://doi.org/10.1016/j.enconman.2024.119117][Neuwirth et al, 2024]]). This allows a complete replacement of
the plant fleet by following their reinvestment cycles ([[https://doi.org/10.1016/j.enconman.2024.119117][Neuwirth et al, 2024]]). For gas networks, which are dominated
by distribution pipes to buildings for heating, many of the assets are also decades old and expected
to be fully depreciated by 2050 ([[https://doi.org/10.1016/j.esr.2022.100908][Bouacida, 2022]]). For remaining newer assets, many European countries have
recently allowed faster depreciation so that gas grid operators can recover their costs before the
pipelines are decommissioned ([[https://doi.org/10.1016/j.erss.2025.103987][Rosenow et al, 2025]]). Decommissioning pipes to make them safe, however, does
represent a substantial one-off cost, estimated to be 25 billion euros in Germany and 29 billion
euros in the United Kingdom ([[https://doi.org/10.1016/j.oneear.2024.05.022][Rosenow et al, 2024]]).
...
#+CAPTION: Global methanol demand in 2019 ([[https://www.irena.org/Publications/2021/Jan/Innovation-Outlook-Renewable-Methanol][IRENA, 2021]])
[[./graphics/methanol_faq-260330/methanol_usage_today.png]]
...
- Improvements to synthesis of competing liquid fuels, e.g. ethanol,
  DME or Fischer-Tropsch.  If all the Fischer-Tropsch products can be
  used (diesel for shipping, kerosene for aviation, naphtha in steam
  crackers for high value chemicals), then FT can substitute methanol
  routes for these sectors.
- Direct electrolysis routes for producing ethylene from water and CO could substitute methanol routes, but they currently have a low TRL.
...
with a GWP100 of 11.6 \pm 2.8 ([[https://doi.org/10.1038/s43247-023-00857-8][Sand et al, 2023]]).

Any leakage of methane or hydrogen could weaken the case for these
energy carriers. There is however a strong economic argument to reduce
leakage rates as much as possible.
...
Hydrogen supply chains could have leakage of up to 4.5% ([[https://doi.org/10.1016/j.ijhydene.2025.150265][Restrepo and
Fulton, 2025]]).


* Is methanol vapour a greenhouse gas?
  :PROPERTIES:
  :CUSTOM_ID: vapour
  :END:

The storage requirements and vapourisation conditions of methanol are
very similar to those of gasoline.

Methanol vapour is not a direct greenhouse gas, but it can indirectly
prolong the lifetime of methane in the atmosphere, and eventually
decays to CO_2 ([[https://forcetechnology.com/en/articles/emissions-ship-engines-methanol-new-green-fuel][Kristensen and Bræstrup]]). This could be a problem if
any vapour leaks from energy systems.

For multiple reasons there are lower risks from methanol vapour
leakage than hydrogen (or methane) leakage: the mass of methanol
vapour compared to consumed liquid is under normal circumstances low;
the vapour pressure is much lower than pressures needed for hydrogen
storage and transportation; the molecule size is smaller; the
atmospheric lifetime is shorter; and the coupling to methane is weaker
than for hydrogen.
...
As discussed above, e-biomethanol and electricity could be produced in discrete cells:
...
* What is the intuition behind methanol's superiority?
  :PROPERTIES:
  :CUSTOM_ID: intuition
  :END:

From a synthesis perspective, the fact that methanol has a single
carbon atom makes it easier to synthesise than other higher alcohols
like ethanol.

Methanol synthesis is highly selective (it produces a relatively pure
product), whereas Fischer Tropsch synthesis produces a wide selection
of hydrocarbons, some of which get wasted or have to be cracked down
to shorter chain lengths. If all the FT products can be used, FT can
be competitive with methanol for producing shipping fuels, kerosene
and naphtha for HVC (the model results show relatively easy
substitution between FT routes and methanol routes for these
products).

Unlike methane or hydrogen, methanol is a liquid at room temperature
and pressure, which makes it much easier to store and handle.

Methanol is already used as a shipping fuel.

There are mature processes used at industrial scale to produce
ethylene and other high value chemicals from methanol ([[https://doi.org/10.1016/j.cherd.2023.04.037][Liang et al,
2023]]).
...
([[https://aireg.de/wp-content/uploads/2024/07/airegWebinar_FT_and_MeOH-based_Kerosene_TUHH.pdf][Bube et al, 2024]]) and the variety of products it produces makes it
less suitable as a platform solution. If all the products can be used
(diesel for shipping, kerosene for aviation, naphtha in steam crackers
for high value chemicals), then FT can substitute methanol routes for
these sectors.
